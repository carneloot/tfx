...
const actualPhotonSha = execFileSync('git', ['rev-parse', 'HEAD'], {
	cwd: specRepo,
	encoding: 'utf8',
}).trim();
if (actualPhotonSha !== pinnedPhotonSha)
	throw new Error(`unexpected Photon SHA: ${actualPhotonSha}`);
...
for (const item of Object.values(document.paths) as Array<
	Record<string, { operationId?: string }>
>) {
	for (const operation of Object.values(item))
		if (operation.operationId) required.delete(operation.operationId);
}
if (required.size > 0)
	throw new Error(
		`required Telegram operations missing: ${[...required].join(', ')}`,
	);
...
const args = [
	...
	...patches.flatMap((name) => [
		'--patch',
		resolve(root, 'packages/tfx/openapi/patches', name),
	]),
];
const generated = spawnSync('corepack', ['pnpm', ...args], {
	cwd: root,
	encoding: 'utf8',
	maxBuffer: 64 * 1024 * 1024,
});
if (generated.status !== 0)
	throw new Error(generated.stderr || `openapigen exited ${generated.status}`);
const normalizeLines = (value: string) =>
	value.replace(/[ \t]+$/gm, '').replace(/\n*$/, '\n');
const warnings = normalizeLines(generated.stderr)
	.trim()
	.split('\n')
	.filter(Boolean);
const warningAllowlist = new Set<string>([]);
const unexpectedWarnings = warnings.filter(
	(warning) => !warningAllowlist.has(warning),
);
if (unexpectedWarnings.length > 0)
	throw new Error(
		`unexpected openapigen warnings:\n${unexpectedWarnings.join('\n')}`,
	);
	...
	for (const operation of Object.values(pathItem)) {
		const properties =
			operation.requestBody?.content?.['application/json']?.schema
				?.properties ?? {};
		for (const [name, schema] of Object.entries(properties) as Array<
			[string, any]
		>) {
			if (
				schema.format === 'binary' ||
				schema.$ref?.endsWith('/InputFile') ||
				/upload|InputFile/i.test(schema.description ?? '')
			)
				multipartFieldNames.add(name);
		}
	}
}
...
normalized = normalized.replaceAll(
	...
	'HttpClientRequest.bodyJsonUnsafe(options.payload as any)',
);
normalized = normalized.replaceAll(
	'HttpClientRequest.bodyFormData(options.payload as any)',
	'bodyTelegramPayload(options.payload as any)',
);
const unexpectedStatusPattern =
	/  const unexpectedStatus = \(response: HttpClientResponse\.HttpClientResponse\) =>[\s\S]*?\n  const withResponse =/;
if (!unexpectedStatusPattern.test(normalized))
	throw new Error('could not replace unexpected status decoder');
normalized = normalized.replace(
	unexpectedStatusPattern,
...
if (!normalized.includes(makeMarker))
	throw new Error('could not insert Telegram request body helper');
...
const target = process.argv[2]
	...
	: resolve(generatedDir, 'TelegramApi.ts');
...
const types = [...full.matchAll(/^export type [^\n]+$/gm)]
	...
	.map((line) => {
		...
		for (const field of multipartFieldNames)
			line = line.replaceAll(
				`readonly "${field}": string`,
				`readonly "${field}": string | Blob`,
			);
		return line;
	});
...
if (interfaceStart < 0 || interfaceEnd < 0)
	throw new Error('could not extract TelegramApi interface');
const apiInterface = full
	.slice(interfaceStart, interfaceEnd + 2)
	.replace(/^  readonly httpClient:.*\n/m, '')
	.replace(/typeof ([A-Za-z0-9_]+)\.(?:Encoded|Type)/g, '$1')
	.replace(
		/<Config extends OperationConfig>\(options: \{ readonly payload: ([^;]+); readonly config\?: Config \| undefined \}\) => Effect\.Effect<WithOptionalResponse<([^,]+), Config>,/g,
		'(options: { readonly payload: $1 }) => Effect.Effect<$2,',
	)
	.replace(
		/<Config extends OperationConfig>\(options: \{ readonly config\?: Config \| undefined \} \| undefined\) => Effect\.Effect<WithOptionalResponse<([^,]+), Config>,/g,
		'(options?: { readonly payload?: Record<string, never> }) => Effect.Effect<$1,',
	);
...
writeFileSync(
	runtimeTarget,
	readFileSync(runtimeTarget, 'utf8').replace(
		/^\/\/ .*TelegramApi\.ts$/m,
		'// generated TelegramApi runtime',
	),
);
writeFileSync(
	resolve(dirname(target), 'TelegramApi.runtime.d.ts'),
...
const manifestFiles = [
	spec,
	...patches.map((name) => resolve(root, 'packages/tfx/openapi/patches', name)),
];
const manifest =
	manifestFiles
		.map(
...
const manifestTarget = resolve(
	root,
	'packages/tfx/openapi/telegram-sources.sha256',
);
