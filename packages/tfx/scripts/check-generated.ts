...
const sourceFiles = [
  ...
  ...["001-server.json", "002-default-responses.json", "003-input-files.json"].map((name) => resolve(root, "packages/tfx/openapi/patches", name))
]
const actualManifest = sourceFiles.map((file) => `${createHash("sha256").update(readFileSync(file)).digest("hex")}  ${file.split("/").at(-1)}`).join("\n") + "\n"
const expectedManifest = readFileSync(resolve(root, "packages/tfx/openapi/telegram-sources.sha256"), "utf8")
if (actualManifest !== expectedManifest) throw new Error("Telegram source/patch SHA256 manifest differs")
