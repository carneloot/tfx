...
describe("CallbackData", () => {
  ...
  it("rejects namespace mismatch and malformed payload", async () => {
    const pet = CallbackData.make("pet", Schema.String)
    await expect(run(pet.decode("food:rex"))).rejects.toMatchObject({ reason: "NamespaceMismatch" })
    await expect(run(pet.decode("malformed"))).rejects.toMatchObject({ reason: "MalformedPayload" })
  })
  ...
  it("counts multibyte payloads as UTF-8 bytes", async () => {
    const value = CallbackData.make("n", Schema.String)
    await expect(run(value.encode("😀".repeat(15)))).resolves.toBe(`n:${"😀".repeat(15)}`)
    await expect(run(value.encode("😀".repeat(16)))).rejects.toMatchObject({ reason: "ByteLimit" })
  })
})
