...
import { InvalidResponseError, NetworkError, TelegramError, fromEnvelope } from "../src/TelegramError.js"
...
describe("TelegramError", () => {
  ...
  it("maps unknown failures and redacts tokens", () => {
    expect(fromEnvelope("call", { ok: false, error_code: 418 }).reason._tag).toBe("UnknownError")
    const network = new TelegramError({ module: "Telegram", method: "getMe", reason: new NetworkError({ message: safeMessage("https://api.telegram.org/bot123456:secret/getMe") }) })
    expect(String(network.reason.message)).not.toContain("123456:secret")
    expect(new InvalidResponseError({ message: "Invalid envelope" }).isRetryable).toBe(false)
  })
