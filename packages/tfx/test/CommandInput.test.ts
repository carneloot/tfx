...
describe("CommandInput", () => {
  it("parses none, argument, optional, repeated, sequence, rest, and map", async () => {
    ...
    expect(await Effect.runPromise(parse(CommandInput.repeated(CommandInput.argument("ids", Schema.NumberFromString)), "1  2 3"))).toEqual({ ids: [1, 2, 3] })
  ...
  it("fails missing input with a typed error", async () => {
    await expect(Effect.runPromise(parse(CommandInput.argument("answer", Schema.String), "")))
      .rejects.toMatchObject({ _tag: "CommandInputError", reason: "MissingInput" })
  })

  it("validates dynamically assembled sequences at runtime", () => {
    const dynamicSequence = CommandInput.sequence as (...inputs: ReadonlyArray<CommandInput.RuntimeInput>) => unknown
    expect(() => dynamicSequence(CommandInput.argument("x", Schema.String), CommandInput.argument("x", Schema.String)))
      .toThrow(CommandInput.CommandInputError)
    expect(() => dynamicSequence(CommandInput.optional(CommandInput.argument("x", Schema.String)), CommandInput.argument("y", Schema.String)))
      .toThrow(CommandInput.CommandInputError)
    expect(() => dynamicSequence(CommandInput.rest("x", Schema.String), CommandInput.argument("y", Schema.String)))
      .toThrow(CommandInput.CommandInputError)
    expect(() => dynamicSequence(CommandInput.rest("x", Schema.String), CommandInput.rest("y", Schema.String)))
      .toThrow(CommandInput.CommandInputError)
  })
