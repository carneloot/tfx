...
describe("keyboard builders", () => {
  it("builds immutable reply rows and options", () => {
    const keyboard = ReplyKeyboard.rows([["Yes", ReplyKeyboard.webApp("Open", "https://example.com")]], {
      isPersistent: true, resize: true, oneTime: true, placeholder: "Choose", selective: true
    })
    expect(keyboard).toEqual({ keyboard: [[{ text: "Yes" }, { text: "Open", web_app: { url: "https://example.com" } }]], is_persistent: true, resize_keyboard: true, one_time_keyboard: true, input_field_placeholder: "Choose", selective: true })
