...
interface DecodeService {
	readonly decode: 'DecodeService';
}
interface EncodeService {
	readonly encode: 'EncodeService';
}
declare const serviced: Schema.ConstraintCodec<
	number,
	string,
	DecodeService,
	EncodeService
...
const value = CommandInput.sequence(
	...
	CommandInput.optional(
		CommandInput.rest('note', {} as Schema.ConstraintCodec<string, string>),
	),
);
...
const noEncodingRequirement: CommandInput.Requirements<
	ReturnType<typeof CommandInput.argument<'x', typeof serviced>>
> = {} as DecodeService;
...
const manyRequirement: CommandInput.Requirements<typeof many> =
	{} as DecodeService;
void manyOutput;
...
CommandInput.sequence(
	// @ts-expect-error duplicate names
	CommandInput.argument('x', serviced),
	CommandInput.argument('x', serviced),
);

CommandInput.sequence(
	// @ts-expect-error required input cannot follow optional input
	CommandInput.optional(CommandInput.argument('x', serviced)),
	CommandInput.argument('y', serviced),
);

CommandInput.sequence(
	// @ts-expect-error input cannot follow rest
	CommandInput.rest('x', serviced),
	CommandInput.argument('y', serviced),
);
