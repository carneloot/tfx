...
import { Conversation, ConversationBuilder, ConversationInput, Conversations, MessageContext } from "tfx"
...
const flow = Conversation.make("flow", {
  ...
  steps: {
    first: Conversation.step("first", { state: Schema.Struct({ count: Schema.Number }), input: ConversationInput.text(Schema.String) }),
    second: Conversation.step("second", { state: Schema.Struct({ name: Schema.String }), input: ConversationInput.text(Schema.String) })
  }
})
const partial = ConversationBuilder.make(flow).step("first", {
  enter: () => Effect.void,
  onInput: (_state, name) => Effect.succeed(ConversationBuilder.to("second", { name }))
})
// @ts-expect-error exhaustive implementation requires second step
ConversationBuilder.done(partial)
const built = ConversationBuilder.done(partial.step("second", {
  enter: () => Effect.void,
  onInput: () => Effect.succeed(ConversationBuilder.complete())
}))
// @ts-expect-error unknown step
ConversationBuilder.make(flow).step("missing", { enter: () => Effect.void, onInput: () => Effect.succeed(ConversationBuilder.complete()) })
...
conversations.start(built, { count: 1 }, { scope: { botId: "b", chatId: 1, userId: 1 } })
// @ts-expect-error startup input is schema-derived
conversations.start(built, { count: "one" }, { scope: { botId: "b", chatId: 1, userId: 1 } })
const contextBuilder = ConversationBuilder.make(flow).step("first", {
  enter: () => Effect.void,
  onInput: () => Effect.as(MessageContext.MessageContext, ConversationBuilder.complete())
})
