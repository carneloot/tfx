...
class Result extends Context.Service<Result, { readonly id: number }>()(
	'types/Result',
) {}
...
const effect = ordered.run(
	Effect.map(CurrentAdmin, (admin) => ({ id: admin.id })),
);
const _infrastructure: Effect.Effect<
	{ readonly id: number },
	Unauthorized,
	UserRepository
> = effect;
...
const _layerRequirement: Layer.Layer<Result, Unauthorized, UserRepository> =
	resultLayer;
