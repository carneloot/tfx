...
export interface BotGroup<Id extends string, Commands extends Readonly<Record<string, Command.Command<any, any, any, any>>>> {
  readonly _tag: "BotGroup"
  readonly id: Id
  readonly commands: Commands
  add<C extends Command.Command<any, any, any, any>>(
    command: C & (C["id"] extends keyof Commands ? never : unknown)
  ): BotGroup<Id, Commands & { readonly [K in C["id"]]: C }>
}

const build = <Id extends string, Commands extends Readonly<Record<string, Command.Command<any, any, any, any>>>>(
  id: Id,
  commands: Commands
  ...
  add<C extends Command.Command<any, any, any, any>>(command: C) {
