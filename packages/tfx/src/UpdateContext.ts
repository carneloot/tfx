...
export class UpdateContext extends Context.Service<UpdateContext, UpdateContextService>()("tfx/UpdateContext") {}
...
export const make = (update: Update): UpdateContextService => {
  ...
  const event = Object.values(root).find((value) => record(value) !== undefined) as unknown
  const item = record(event)
  const callbackMessage = record(record(root.callback_query)?.message)
  const message = record(root.message) ?? record(root.edited_message) ?? record(root.channel_post) ?? record(root.edited_channel_post) ?? record(root.business_message) ?? record(root.edited_business_message) ?? callbackMessage
  const user = record(item?.from) ?? record(item?.user) ?? record(message?.from)
