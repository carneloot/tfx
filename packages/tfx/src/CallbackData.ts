...
export class CallbackDataError extends Error {
  ...
  constructor(readonly reason: "InvalidNamespace" | "NamespaceMismatch" | "MalformedPayload" | "ByteLimit", message: string) {
    super(message)
  }
}
...
export type DecodingServices<T> = T extends CallbackData<any, infer R, any> ? R : never
export type EncodingServices<T> = T extends CallbackData<any, any, infer R> ? R : never
...
export const make = <A, RD, RE>(
  ...
  const encode = (value: A) => Schema.encodeEffect(codec)(value).pipe(
    Effect.flatMap((payload) => {
      ...
      return bytes >= 1 && bytes <= 64
        ...
        : Effect.fail(new CallbackDataError("ByteLimit", `Callback data must be 1-64 UTF-8 bytes; received ${bytes}`))
    })
  )
  const decode = (value: string) => {
    ...
    if (value.slice(0, split) !== namespace) {
      return Effect.fail(new CallbackDataError("NamespaceMismatch", `Expected callback namespace '${namespace}'`))
    }
...
export const registry = <const C extends ReadonlyArray<CallbackData<any, any, any>>>(...codecs: C) => {
  ...
  for (const codec of codecs) {
    if (values[codec.namespace] !== undefined) {
      throw new CallbackDataError("InvalidNamespace", `Duplicate callback namespace '${codec.namespace}'`)
    }
