...
export class VersionedSchemaError extends Error {
	...
	constructor(
		readonly reason: 'InvalidVersion' | 'MissingMigration' | 'DecodeFailure',
		message: string,
...
export interface Version<V extends number, A> {
	readonly version: V;
...
export const version = <const V extends number, A>(
	...
	if (!Number.isInteger(value) || value <= 0)
		throw new VersionedSchemaError(
			'InvalidVersion',
			`Version must be a positive integer; received ${value}`,
		);
	return Object.freeze({ version: value, schema });
};
const build = <V extends ReadonlyArray<Version<number, any>>>(
	versions: V,
	...
	const self: History<V> = {
		...
		latest,
		migrate,
		decode: (value) => migrate(value.version, value.state),
		pipe: (f) => f(self),
	};
	return Object.freeze(self);
};
export const history = <V extends Version<number, any>>(
	first: V,
): History<readonly [V]> => build([first], []);
export const to =
	<
		V extends ReadonlyArray<Version<number, any>>,
		Next extends Version<number, any>,
	>(
		next: Next,
		migrate: (
			value: Last<V> extends Version<number, infer A> ? A : never,
		) => Next extends Version<number, infer B> ? B : never,
	) =>
	(self: History<V>): History<readonly [...V, Next]> => {
		const previous = self.latest.version;
		if (next.version !== previous + 1)
			throw new VersionedSchemaError(
				'InvalidVersion',
				`Expected version ${previous + 1}; received ${next.version}`,
			);
		return build(
			[...self.versions, next],
			[...self.migrations, { from: previous, to: next.version, migrate }],
		);
	};
