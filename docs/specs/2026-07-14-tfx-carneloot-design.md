...
An event stores kind, owner, optional template and pet references, and creation time. A delivery stores event, recipient user/chat, recipient role (`owner` or `subscriber`), optional Telegram message ID, status (`pending`, `sending`, `sent`, `failed`, or `unknown`), attempt generation/count, sending lease timestamps, safe error summary, and outcome timestamps. Sent Telegram identity is unique by `(bot_id, recipient_chat_id, telegram_message_id)`.

Before sending, Carneloot transactionally creates event plus `pending` recipient deliveries. `pending` strictly means no Telegram attempt has started. Each sender atomically claims one eligible pending or explicitly retryable failed row by changing it to `sending`, incrementing attempt generation/count, and setting sending start/lease before making HTTP call. Completion updates require same generation.

Definitive Telegram success plus persisted message ID becomes `sent`. Definite API rejection becomes `failed` with typed safe/permanent classification; only known-safe retryable failures such as explicit rate-limit rejection may be scheduled again. Ambiguous transport failure, interruption during send, or stale expired `sending` becomes `unknown`, because Telegram may have accepted request. `sent` and `unknown` are never automatically retried. No Telegram send starts if initial event/delivery transaction fails.

Crash after committing `sending` but before actual HTTP call conservatively becomes unknown, potentially losing one delivery to prevent duplicate. If original process later obtains definitive success before any newer generation exists, matching-generation late reconciliation may change `unknown` to `sent`; explicit resend creates new generation and fences stale completion.

Generic notification response includes event ID, status (`sent`, `partial`, `failed`, or `indeterminate`), counts for sent/failed/unknown, and sanitized failure summaries. Any delivery still `sending` when response wait ends contributes to response's unknown count while persisted lease recovery determines final row state; a recipient never attempted during send phase is finalized as failed rather than left pending. Final mapping is exact: all recipients persisted sent returns `200 sent`; at least one sent plus any failed/unknown returns `207 partial`; zero sent with known failures and no unknown returns `502 failed`; zero sent with any unknown returns `202 indeterminate`. Initial event/pending-delivery transaction failure returns `503` and sends nothing, so caller may retry safely. Post-send status-write or final reread failure never returns generic retryable database error: it contributes unknown and yields `207` or `202` because blind caller retry could duplicate accepted Telegram messages. Unexpected application defect remains `500`.
...
Job-based notification delivery evaluates recipient rows independently: `sent` and `unknown` skip, `pending` attempts, retryable `failed` follows policy, and permanent `failed` skips. Whole reminder batch never resends recipients already sent or ambiguous. Recovery converts expired `sending` to `unknown` rather than pending, so crash after Telegram accepts message cannot become automatic duplicate retry.
...
- notification event/pending-delivery creation, fenced `pending → sending → outcome` transitions, crashes before/after HTTP, definite versus ambiguous failure, stale sending recovery to unknown, late reconciliation, retry skipping for sent/unknown, exact `200`/`207`/`202`/`502` matrix, initial `503` without sends, final reread failure, exact owner-event reply correlation, and chat-scoped message identity;
- importer validation.
