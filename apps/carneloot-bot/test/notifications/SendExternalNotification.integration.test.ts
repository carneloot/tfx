...
if (!enabled)
	describe.skip('SendExternalNotification PostgreSQL', () =>
		it('requires database', () => {}));
else
	describe('SendExternalNotification PostgreSQL', () => {
		it('persists frozen external event and initial recipient delivery atomically', async () => {
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					return yield* notifications.createExternalEvent(
						{
							id: eventId(),
							botId,
						...
						{ templateId: null, renderedMessage: 'frozen message' },
						[
							{
								_tag: 'Reachable',
								id: deliveryId(),
								recipientUserId: registered.user.id,
								recipientChatId: registered.profile.privateChatId,
								recipientRole: owner,
								channel: 'telegram',
							},
						],
			...
			expect(result.deliveries).toMatchObject([
				{ status: 'pending', recipientChatId: expect.any(Number) },
			]);
		});
	});
