...
const CaregiverState = Schema.Struct({ ...Base, petId: PetId, petName: PetName, caregivers: Schema.Array(CaregiverOption) });
...
const caregiverChoice = (items: ReadonlyArray<typeof CaregiverOption.Type>) => ConversationChoice.make(items.map((item) => ({ label: item.label, value: item.id })));
const prompt = <A>(text: string, options: ConversationChoice.Choice<A, never>) => Effect.flatMap(MessageContext.MessageContext, (context) => context.reply(text, { reply_markup: { keyboard: options.options.map((item) => [{ text: item.label }]), one_time_keyboard: true, resize_keyboard: true } })).pipe(Effect.asVoid);
...
const output = (notices: ReadonlyArray<{ readonly chatId: number; readonly text: string }>) => Effect.gen(function* () { yield* reply('Cuidador removido com sucesso!', true); const telegram = yield* Telegram.Telegram; yield* Effect.forEach(notices, (notice) => telegram.sendMessage({ chat_id: notice.chatId, text: notice.text }), { discard: true }); });

export const declaration = Conversation.make('remove-pet-caregiver', { version: 1, startup: PetState, initialStep: 'pet', initialize: (state) => state, steps: { pet: Conversation.step('pet', { state: PetState, input: Text }), caregiver: Conversation.step('caregiver', { state: CaregiverState, input: Text }) }, idleTimeout: 15 * 60 * 1000, error: ApplicationError });
export const built = ConversationBuilder.done(ConversationBuilder.make(declaration)
	.step('pet', {
		enter: (state) => required(prompt('Escolha o pet:', petChoice(state.pets))),
		onInput: (state, value) => required(Effect.gen(function* () { const selected = yield* Effect.result(ConversationPrompt.resolve(petChoice(state.pets), value)); if (selected._tag === 'Failure') return yield* stay(); const resolved = selected.success; if (resolved._tag === 'Cancelled') return yield* stay(); const pet = state.pets.find((item) => item.id === resolved.value); if (pet === undefined) return yield* stay(); const listed = yield* Effect.result(ListCaregivers.execute(state, pet.id)); if (listed._tag === 'Failure' && listed.failure._tag === 'CaregiverAccessLost') return unavailable(); if (listed._tag === 'Failure') return yield* Effect.fail(listed.failure); if (listed.success.length === 0) return ConversationBuilder.complete({ afterCommit: reply(`O pet ${pet.name} não possui cuidadores.`, true) }); return ConversationBuilder.to('caregiver', { ...state, petId: pet.id, petName: pet.name, caregivers: listed.success.map((item) => ({ id: item.relation.caregiverUserId, label: `${item.displayName} (${item.statusLabel})` })) }); })),
		onInvalid: () => stay(),
	})
	.step('caregiver', {
		enter: (state) => required(prompt('Escolha a pessoa cuidadora que deseja remover:', caregiverChoice(state.caregivers))),
		onInput: (state, value) => required(Effect.gen(function* () { const selected = yield* Effect.result(ConversationPrompt.resolve(caregiverChoice(state.caregivers), value)); if (selected._tag === 'Failure') return yield* stay(); const resolved = selected.success; if (resolved._tag === 'Cancelled') return yield* stay(); const result = yield* Effect.result(RemoveCaregiver.execute(state, state.petId, resolved.value)); if (result._tag === 'Failure' && (result.failure._tag === 'CaregiverAccessLost' || result.failure._tag === 'CaregiverInvitationNotFound')) return unavailable(); if (result._tag === 'Failure') return yield* Effect.fail(result.failure); return ConversationBuilder.complete({ afterCommit: output(result.success.notices) }); })),
		onInvalid: () => stay(),
	}));
