...
export const execute = Effect.fn('SendExternalNotification.execute')(function* (
	...
	const ownerUserId = yield* keys
		.findUserIdByHash(keyHash)
		.pipe(
			Effect.mapError(() => new InvalidApiKey({ message: 'Invalid API key' })),
		);
	...
	const resolved = yield* templates
	...
	if (rendered instanceof MissingTemplateVariables)
		return yield* Effect.fail(rendered);
	...
	const reachable = created.deliveries.filter(
		(delivery) =>
			delivery.recipientChatId !== null && delivery.status === 'pending',
	);
