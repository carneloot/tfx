...
export const execute = Effect.fn('InviteCaregiver.execute')(
	(actor: CaregiverActor, petId: PetId, username: string) =>
		Effect.gen(function* () {
			...
			return yield* sql.withTransaction(
				Effect.gen(function* () {
					...
					if (normalized.length === 0)
						return yield* Effect.fail(
							new CaregiverUsernameNotFound({
								message: 'Caregiver username is empty',
							}),
						);
					...
					if (matches.length === 0)
						return yield* Effect.fail(
							new CaregiverUsernameNotFound({
								message: 'Caregiver username was not found',
							}),
						);
					if (matches.length !== 1)
						return yield* Effect.fail(
							new CaregiverUsernameAmbiguous({
								message: 'Caregiver username is ambiguous',
							}),
						);
					...
					if (invitee.user.id === actor.actorId)
						return yield* Effect.fail(
							new CaregiverSelfInvitation({
								message: 'Owner cannot invite self',
							}),
						);
					const relation = yield* caregivers.insertPending(
						petId,
						invitee.user.id,
						yield* DateTime.now,
					);
					return {
						...
						notices: [
							{
								chatId: invitee.profile.privateChatId,
								text: `${displayName(owner)} convidou você para cuidar do pet ${pet.name}.\nUse /convites_pet para responder.`,
							},
						],
