...
export const execute = (actor: CaregiverActor, petId: PetId) => Effect.gen(function* () {
	...
	return yield* sql.withTransaction(Effect.gen(function* () {
		...
		return yield* Effect.forEach(yield* caregivers.listForPet(petId), (relation) =>
			Effect.map(users.findById(actor.botId, relation.caregiverUserId), (user) => ({ relation, displayName: displayName(user), statusLabel: statusLabel(relation.status) })),
		);
	}));
});
